...
{
    // Extrae la posición de la cuarta columna de `modelMatrix`
    glm::vec3 targetPosition = glm::vec3(modelMatrix[3]);

    // Ajusta la posición de la cámara con el desplazamiento especificado
    position.x = targetPosition.x + offsetX;
    position.y = targetPosition.y + offsetY;
    position.z = targetPosition.z + offsetZ;

    // Actualiza `yaw` según la dirección del avatar
    yaw = dirAvatar;
